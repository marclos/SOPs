...
\NP Originally, this SOP was developed for greenhouse gas emissions from strawberry fields in NorCal. It has been been modified as new projects come on line that rely on these instruments. 
...
\NP Sampling chambers should be randomly located and within 30 meters of the analyzer/multipler.

\NP The cable and gas tubing are 30 meters and labelled with letters. Bad cables should be noted immediately and removed from the inventory and repaired. 

\NP If the generator is used, place it downwind at a minium of 100 feet.

\section{Set Up}

\NP Connect pump to Analyzer VACUUM port using the corrogated metal hose, using new washers each time (Swagelock Part \#). Hand tighten and use wrench to seal connection.
