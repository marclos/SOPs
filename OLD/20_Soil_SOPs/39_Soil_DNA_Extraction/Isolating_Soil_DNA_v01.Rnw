...
\NP Pipettor 50 $\mu$L - 750 $\mu$L and 20 $\mu$L - 200 $\mu$
...
\NP Disposable pipette tips (for 100 preps) 
...
\section{Micropipetting}
This procedure will utilize two pipettes: 50 $\mu$L - 750 $\mu$ and 20 $\mu$L - 200 $\mu$. The second pippette will only be used near the end of the procedure for a more precise sampling. Regardless, it is important to familarize yourself with how to use a pipette. Ensure that they are properly calibrated by consulting with Marc. Afterwards, you are ready to pippette.
\NP You will notice that on the side of the pipette there are three numbers stacked vertically. This is where you read the volume of the micropipette. To adjust the volume, you turn the plunger and this will change the extraction volume. \begin{figure}[h!]
  \centering
  \includegraphics[width=0.6\textwidth]{pipe.png}
  \caption{Here is an example of how to read the pipette}
  \label{fig:Pipette Standard Reading}
\end{figure}
\NP Once you have turned the wheel to the desired extraction volume, the micropipette will extract exactly this amount from whatever concentrate you pull from. 
\NP Ensure you have the proper tip for the micropipette. Now, Insert the tip into your concentrate push down on the plunger until you reach a stopping point. Then, release back up. You have now extracted your concentrate. R
\NP Now that the concentrate is pulled from the pipette, push down, all the way into a stopping point to release it into your intended sample. You can practice this extraction and release process with water before formally extracting.
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.6\textwidth]{pos.jpg}
  \caption{Second position is the extraction and third position is releasing the extract}
  \label{fig:Pipette Position}
\end{figure}

\section{Estimated Time (batch of 24)}
...
\NP Before the extraction process, soils should be freshly collected.
...
\NP Pull a beaker to use for waste during experiment
...
\NP Centrifuge the PowerBead Pro Tube at 15,000 x g for 1 min. \textbf{Make sure to place samples across from each other in the centrifuge, otherwise it will not work as it is imbalanced. Additionally the machine will say 15 for 15,000 x g, 14 for 14,0000 x g, and so on.}
\NP Transfer 500-600 $\mu$L of the supernatant to a clean 2 ml Microcentrifuge Tube (provided). \textbf{Make sure to number the top of every tube as the side will later be covered up in following steps} Avoid disturbing the pellet. There may also still be soil particles.
...
\NP Centrifuge at 15,000 × g for 1 minute. Discard the flow-through in your waste beaker.
...
\NP Discard the flow-through in your waste beaker and place the MB Spin Column back into the same 2 ml Collection Tube.
...
\textbf{All waste is hazardous waste disposal EXCEPT used spin columns}
\begin{itemize*}
  \item Buffer CD1: Guanidine thiocyanate (chaotropic salt)
  \item Buffer CD2: Detergents
  \item Buffer EA: Ethanol or isopropanal
  \item Buffer C5/C6/EA: tris buffer + ethanol
  \item Flowthrough from spin columns: Mix of all listed above
  \item Used spin columns: Residual chaotropes & DNA. \textbf{Note: This will be solid hazardous lab waste}
