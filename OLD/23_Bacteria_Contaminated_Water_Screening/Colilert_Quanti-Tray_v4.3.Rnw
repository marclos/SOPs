...
\NP To determine the number of total coliforms, use the table in the appendix (small wells only). We will also use the IDEXX software. Or try the software provided by the IDEXX -- \url{https://www.idexx.com/en/water/resources/mpn-generator/} (windows only)
...
\NP Report results in the label with a permanent for both the small and large cells.


\NP Use the Appendix Tables to calculate as an approximate MPN. But to generate the MPN, we'll need to use IDEXX software software to double check the results.
