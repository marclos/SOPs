...
\begin{enumerate}[label=(\alph*)]
  \item If you are only downloading data and not using the Picarro for experimental cycles, you do not neccesarily need to        wait for the Picarro system that pops up on the screen to warm up. 
    %\item If the program is not on the desktop, open the Start Menu and search "eosAnalyze-AC." Open the "eosAnalyze-AC_v3.4     .2" folder. In the folder, open the .exe application with the same name. (Some bit of this won't let the pdf compile and        idk what)
\end{enumerate}
...
%\NP If the data is not loading into the program or you get a "No measurements detected" message, try resetting the data path. Go to \textit{Data> Analyzer Data Path}. The data path folder should be %C:\UserData\DataLog_Forerunner\. (how to fix this so it's formatted correctly ??)
You should see subfolders named by year. Click OK, then retry collecting data. 
%\begin{itemize}
      %\item If you still cannot get data, use Windows Explorer on the taskbar to search for the data logs. Double-check %C          :\UserData\DataLog_Forerunner\YEAR\MONTH\DATE
     % folder for .DAT files. The naming convention should be something like this: JFAADS2048-20180803-131415-DataLog_Forerunne       r.dat
      %\begin{enumerate}
...
\NP Days that should have more data: 
...
\NP Days where some chambers are missing: 
