...
%\NP If the data is not loading into the program or you get a "No measurements detected" message, try resetting the data path. Go to \textit{Data/Analyzer Data Path}. 

%The data path folder should be \url{C:\UserData\DataLog_Forerunner\}. 
