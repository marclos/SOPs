...
\NP If the data is not loading into the program or you get a "No measurements detected" message, try resetting the data path. Go to \textit{Data/Analyzer Data Path}. The data path folder should be \url{\backslash UserData \backslah DataLog_Forerunner\}. 
You should see subfolders named by year. Click OK, then retry collecting data. 
\begin{itemize}
      \item If you still cannot get data, use Windows Explorer on the taskbar to search for the data logs. Double-check \url{\backslash UserData \backslash DataLog_Forerunner\backslash YEAR \backslash MONTH \backslash DATE} 
