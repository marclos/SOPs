...
\NP Prepare the soil samples in advance. Samples should be sieved with a 2mm sieve.
  Use a drying oven to remove moisture content from the soil and the crucibles (optional) that may affect the true dry weights.  
  
  \begin{enumerate}
	  \item Weigh out more sample mass than desired on an metal tin, make sure to label (etch with pencil) and tare out weight of the tin. %Record the air-dry mass for reference if necessary.
	  \item Oven-dry tins at 105$^\circ$C for 24 hours. The oven can fit X samples.
	  \item Take samples out of the oven using tongs or gloves after oven-drying. Cool them in a desiccator.
	  \item (Optional) Oven-dry crucibles in the same manner. Weigh them once finished while still warm.  
	\end{enumerate}
	
\subsection{Ignition}

\NP Once ready for ignition, we can begin heating the oven while preparing the crucibles. The Geology Dept. has multiple muffled furnaces. This SOP instructs how to use the Nabertherm furnace and load it while hot.
...
\NP Then, for each sample...
  \begin{enumerate}
	\item Record the weight of the crucible on a scale capable of precision to 0.0001
	\item Weigh out 5.0000g ±0.0001g of the oven-dried sample into a crucible.
	\item Record this as the pre-ignition weight. 
		\end{enumerate}
		
\NP As you prepare the crucibles, use the Combustion Residue Mapping Sheet to map out assign them Sample IDs, and correspondingly where you'll put each sample in the furnace. This is important to prevent mixing up samples as any marker or pencil written on the crucibles will burn off. Use the hole on the plate to help you determine direction.

\NP Once the furnace is ready, use the tongs and heat-proof gloves to place the samples according to your map. 
