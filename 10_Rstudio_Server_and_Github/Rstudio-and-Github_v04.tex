...
\NP Select 'RSA" under 'SSH Key Type' drop down. 

\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{graphics/Updated_creating_SSH.png}
\caption{After natigating through the Rstudio Git/SNV menu, select `Create an SSH Key,' then select 'RSA' under the SSH key type drop down.}
\end{figure}
...
\NP To link a Github repository in Rstudio, you will need to ``clone'' the site, while we create a new project in R studio. Unfortunately, the a new interface in github, has a button that labeled ``code'', but that's an easy work around.\label{Linking}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{graphics/Github_GreenCodeButton.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{graphics/CloningGithub2.jpg}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{graphics/stagedcheckboxes.png}
...
\begin{figure}
\centering
\includegraphics[width=0.8\textwidth]{graphics/MgaV9}
\end{figure}
...
\begin{enumerate}
\item Create a New Repository (if necessary) in Github
\item Clone the respository using the steps starting from \ref{Linking}
\item Pull 
...
\subsectiion{'fatal: Not possible to fast-forward, aborting.'}
...
\NP Go to Profile, repository, toolbar in repository click settings, click on collaboration. Add usernames of collaborators to give them permission to collaborate.
