...
\begin{itemize}
  ...
  \item Then go to RStudio and create a new project
    \item File -> new project -> version control -> Git
  ...
  \item In the shell type the following code
    \begin{verbatim}
    \item Git clone (your SSH link you coped from GitHub earlier)
    \item Git remote -v
    \end{verbatim}
    ...
    \item Next we need to add Marc’s repository to this project so that you can push and pull later on
      ...
      \item Back in the shell, type the code below
        \begin{verbatim}
        \item Git remote add upstream (the SSH key you just copied)
        \item Git remote -v
        \end{verbatim}
