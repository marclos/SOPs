...
\begin{itemize}
  ...
  \item Once that finishes, look at the bar at the top of your screen until you find the ``tools'' button, click it and then click shell
...
\begin{itemize}
\item open a new terminal by looking for the ``tools'' button along the top of your screen and then clicking ``terminal'', and then ``new terminal''. Then type the following lines of code

  \indent\verb git clone -b EA30e21 --single-branch <past marc's repository url>
...
\begin{figure}[htbp]
\centerline{\includegraphics{terminaloutput.jpg}}
\caption{it should yield this result in yout terminal}
\label{figure 1}
\end{figure}
...
\item to pull from the EA30e21 branch type the following line of code
...
\item Write the following code in the shell, pressing enter before each new bullet point 
  ...
  \indent\verb"Git pull upstream master"
