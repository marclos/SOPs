...
\NP The probe can be used to take the above measurments in bodies of water, including lakes, streams, and rivers. 
...
\NP {Cable to connect Eureka probe and Amphibian}
...
\NP Calibration of each sensor requires similar procedures. First, turn on the Amphibian and press the Windows icon in the lower left corner. Then, scroll down to the bottom of the main page and select ----. Once the interface loads, press ``Manta2" on the lower left bar, and select ``Standardize" from the menu that appears. Select the sensor you would like to standardize. 

\NP Ex: Standardizing pH. To standardize the pH meter, follow the steps above and select ``pH units" from the Manta2 menu. The screen will instruct you to input the value of your first standard. Then, unscrew just the blue cap from the larger cap of the probe, leaving the clear plastic cup surrounding the sensors in place. Pour a small amount of your first standard into the cup (with the probe facing upward), place the lid on, and shake, to rinse in the standard. Do this twice in total, and then pour the standard into the cup, enough to cover all sensors. Once the red line on chart of the Amphibian has stabalized (is coming out flat), press ``Ok". The screen will now prompt you to put in your second standard. Rinse and shake twice with DI water, and then twice with your second standard, before filling it above the sensors and repeating your process from above. When prompted, select the option to do a three-point calibration, and repeat these steps (including two rinses with DI and then with the standard) with your third standard.
