...
\NP The centrifuge should always be carried by at least two people due to its weight. ALways lift the centrifuge at both sides.

\NP To prevent possible injuries at least two people should lift and carry the centrifuge by holding it at the bottom from opposite sides.

\subsection {Leveling}

\NP The centrifuge should be placed on horizontal and level supporting structures or benching. If it isn't leveled, the centrifuge can be damaged.

\NP Horizontal level is to be checked when moving the centrifuge to a new location.

\subsection {Mains Connection}

\NP Plug the centrifuge into grounded electrical sockets only.

\NP The power supply switch is on the backside, bottom right of the Sorvall ST 8

\NP The Power supply switch is on the right side, top left of the SOrvall ST 8R

\NP Make sure the voltage and frequency correspond to the figures on the rating plate.

\NP Establish the connection to the power supply with the connecting cable. 
\subsection {Storage}

\NP Centrifuge, rotors, buckets and accessories must be disinfected, decontaminated and thoroughly dried before storate.

\NP Sotre the centrifuge in a clean, dust-free location.

\NP Be sure to place the centrifuge on its feet.

\NP Avoid Direct Sunlight.

\subsection {Operation}

\subsection {Switching on the Centrifuge}

\NP Turn on the power switch of the device.

\NP The device performs a self-check of its software. The speed and time displays read "0" and "00:00"; the current temperature inside the rotor chamber is displayed.

\NP When the centrifuge door is open, the display shows the speed and time which show the pre-set values. The set temperature inside the rotor chamber is displayed.

\NP Open the centrifuge door by pressing the OPEN key

\NP Close the centrifuge door by pressing down on it lightly in the middle or on both sides of it. The centrifuge door mechanism will click and lock in place. Lids should not be slammed as excessive force may cause damage or disrupt samples.
