...
\usepackage{Sweave}
\begin{document}
\input{Soil_Texture_Analysis_160725-concordance}
...
\begin{figure}
\includegraphics{Soil_Texture_Analysis_160725-USDATT}
\end{figure}
...
\begin{figure}
\includegraphics{Soil_Texture_Analysis_160725-simplifiedgraph}
...
\begin{figure*}
\includegraphics{Soil_Texture_Analysis_160725-TTPlot}
...
Soils are defined as particles that are less than 2 mm. Thus, we begin by removing the gravel, particles that exceed the sand-sized particles, >2.0 mm. These particles, however, influence the soil texture, so we will quantify the contribution of particles that exceed 2 mm. 
...
Particles retained on the No. 200 sieve are larger than XX in diameter. But collecting the particles on the No. 200 sieve, we \ldots...

we will quantify the contribution of particles that exceed xx . 

We can do this by analyzing the mass of the soils retained on the No. 200 sieve, which has a mesh size of XX mm.  
...
\begin{enumerate}
	...
	\item Select a soil tin, and record the tin No. (Box XX) and tin mass (Box XX). 
...
Record the results in Appendix A, Then enter the data into the data entry form (\href{http:\\test}{test}. Other columns calculations will be produced with the R code from Appendix B.  
