...
goodKEYs <- unique(joined3$sampleKEY); goodKEYs
#KEY <- KEY[-c(1:25, 47)]; KEY

variables = 4 
iterations = length(goodKEYs)
textureclasses <- matrix(ncol=variables, nrow=iterations)
 
for(i in 1:iterations){
tmp3 = joined[joined3$sampleKEY==goodKEYs[i],]; head(tmp3)
tmp3 = select(joined3, sampleKEY, De, PF) %>% filter(joined3$sampleKEY==goodKEYs[i]); tmp3 

Perclay = round(Interpolation(tmp3, clay),1)
Persilt = round(Interpolation(tmp3, silt), 1)
Persand = round(100-(Persilt + Perclay), 1)
textureclasses[i,] <- c(goodKEYs[i], Persand, Persilt, Perclay)
}
