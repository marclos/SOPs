...
The Shimadzu TOC-SSM system uses high-temperature combustion (900°C) to convert all carbon in the sample to CO$_2$, which is then quantified by NDIR (Non-Dispersive Infrared) detection. For carbonate-bearing soils:
...
Prepare from dried calcium carbonate (CaCO$_3$):
\begin{itemize}[noitemsep]
    \item Calculate carbon content: 12.0\% C by weight
    \item Prepare solid standards: 1, 5, 10, 25, 50 mg CaCO$_3$
...
\begin{enumerate}[noitemsep]
    \item Weigh CaCO$_3$ standards directly into ceramic boats
...
\begin{enumerate}[noitemsep]
    ...
    \end{itemize}
    \item Start analysis sequence
    \item Acid converts carbonates to CO$_2$ at low temperature
