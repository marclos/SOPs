...
check_sop <- function(path) {
  ...
  # Find the line index of ## References (may not exist)
  refs_line <- which(str_detect(txt, "^## References\\s*$"))
  
  plain_refs_list <- FALSE
  if (length(refs_line) == 1L) {
    start  <- refs_line + 1L
    end    <- min(refs_line + 10L, length(txt))
    if (start <= end) {
      window <- txt[seq(start, end)]
      window <- window[nchar(trimws(window)) > 0]   # drop blank lines
      if (length(window) > 0) {
...
cat("\n=== NON-COMPLIANT SOPs ===\n\n")
audit |>
  filter(!has_refs_div | !has_definitions | plain_refs_list) |>
  arrange(file) |>
  print(n = Inf)
...
cat(sprintf("Plain-text refs detected: %d\n", sum(audit$plain_refs_list)))